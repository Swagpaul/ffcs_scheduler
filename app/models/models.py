...
class User(db.Model):
    ...
    professors = db.relationship('Professor', backref='user', lazy=True, cascade="all, delete-orphan")
    timetables = db.relationship('GeneratedTimetable', backref='user', lazy=True, cascade="all, delete-orphan")
...
class Professor(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True, index=True)
    name = db.Column(db.String(100), nullable=False)
    courses = db.relationship('CourseProfessor', backref='professor', lazy=True, cascade="all, delete-orphan")
